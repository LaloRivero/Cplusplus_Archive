...
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -O2 -g -Wl,-e,__entry -Wno-unused-result -Ilibinteractive/Main
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\Bubble\Bubble.exe
SET SOURCES=Bubble.cpp libinteractive\Bubble\Bubble_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -O2 -g -Wno-unused-result -Ilibinteractive/Bubble
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\Bubble\Bubble_debug.exe
SET SOURCES=Bubble.cpp libinteractive\Bubble\Bubble_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -g -Wno-unused-result -Ilibinteractive/Bubble
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
...
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GCC% -o %TARGET% %SOURCES% -std=c99 -O2 -lpsapi -Wall
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
